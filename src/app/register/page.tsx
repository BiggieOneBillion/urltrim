...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      let message = await register(name, email, password); // Use the register method from context
      console.log(message);
    } catch (err) {}
  };
  ...
  return (
    <div className="min-h-screen text-black flex flex-col justify-center items-center background py-6 px-4 sm:px-6 lg:px-8">
      <Head>
        <title>Sign Up</title>
      ...
      <div className="w-full mt-1 md:mt-24 sm:w-1/3 montserrat bg-white shadow-md rounded-lg p-3 sm:p-8 border border-gray-200">
        ...
        <form className="mt-3 space-y-2" onSubmit={handleSubmit}>
          ...
          <Button
            ...
            isLoading={loading}
            variant="black"
            className="w-full py-1.5 font-bold text-sm bg-black hover:bg-gray-800"
          >
            Sign up
          </Button>
        </form>

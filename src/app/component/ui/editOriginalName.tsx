// app/component/ui/editOriginalNameUrlModal.tsx
import React, { useState } from "react";
import {Spinner} from "@/app/component/ui"
...
export const EditOriginalName: React.FC<editOriginalNameProps> = ({
  ...
  const currentUrl = currentOriginalName
  ...
  const handleeditOriginalName = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    ...
    if (!validIdPattern.test(newOriginalName)) {
      setError(
      "Original URL can only contain letters, numbers, slashes (/), dots (.), colons (:), hyphens (-), and underscores (_)"
    );
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || "Failed to editOriginalName URL");
        setIsSubmitting(false);
        return;
      }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 px-4">
      <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg sm:text-xl font-bold">Edit Original URL</h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-4 sm:mb-6">
          ...
          <p className="text-xs sm:text-sm text-gray-500 italic">
            Enter a new Url. This will change the URL
            that users visit.
          </p>
        </div>

        <form onSubmit={handleeditOriginalName}>
          <div className="mb-4">
            <label
              htmlFor="newOriginalName"
          ...
          <div className="flex justify-between gap-2">
            ...
            <button
              ...
              disabled={isSubmitting}
            >
              {isSubmitting ? <Spinner color='white'/>: "edit URL"}
            </button>
          </div>
        </form>
      </div>
